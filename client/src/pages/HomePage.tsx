...
export function HomePage() {
  ...
  const [because, setBecause] = useState<Awaited<ReturnType<typeof insightsApi.becauseYouWatched>>>(null);
  ...
  useEffect(() => { if(!canLoadPrivate){setRecommendations([]);return;}insightsApi.recommendations().then((items) => setRecommendations(items.map((item) => item.anime))).catch(() => setRecommendations([])); }, [canLoadPrivate]);
  useEffect(() => { if(!canLoadPrivate){setStats(null);return;}insightsApi.stats().then(setStats).catch(() => setStats(null)); }, [canLoadPrivate]);
  useEffect(() => { if(!canLoadPrivate){setBecause(null);setCompleted([]);return;}insightsApi.becauseYouWatched().then(setBecause).catch(() => setBecause(null)); progressApi.recentlyCompleted().then(setCompleted).catch(() => setCompleted([])); }, [canLoadPrivate]);
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-page px-4 pb-12 sm:px-6 lg:px-8 xl:px-10">
        ...
        {recommendations.length > 0 && <AnimeSection title="For You" subtitle="Picked from your local library" anime={recommendations} loading={false} viewAllLink="/browse" />}
        {because && because.recommendations.length > 0 && <AnimeSection title={`Because You Watched ${because.sourceAnime.title}`} subtitle="Related picks based on your completed episodes" anime={because.recommendations.map((item) => item.anime)} loading={false} viewAllLink="/browse" />}
