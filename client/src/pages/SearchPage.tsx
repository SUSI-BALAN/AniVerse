  ...
  return (
    <div className="page-shell min-h-[70vh]">
      <header className="mx-auto max-w-3xl pt-4 text-center sm:pt-8">
        ...
        <div className="relative mt-8 text-left"><SearchBar value={query} onChange={updateQuery} autoFocus pending={pending} suggestionsOpen={suggestionsOpen && trimmedQuery.length >= 2 && suggestions.length > 0} activeSuggestion={selectedSuggestion} onKeyDown={(event) => { if (event.key === "Escape") { setSuggestionsOpen(false); setSelectedSuggestion(-1); } else if (event.key === "ArrowDown" || event.key === "ArrowUp") { event.preventDefault(); setSuggestionsOpen(true); setSelectedSuggestion((i) => event.key === "ArrowDown" ? (i + 1) % suggestions.length : (i <= 0 ? suggestions.length - 1 : i - 1)); } else if (event.key === "Enter" && suggestionsOpen && selectedSuggestion >= 0 && suggestions[selectedSuggestion]) { event.preventDefault(); const value = suggestions[selectedSuggestion].query; setParams({q:value}); setQuery(value); setPage(1); setSuggestionsOpen(false); setSelectedSuggestion(-1); } }} />{suggestionsOpen && trimmedQuery.length >= 2 && suggestions.length > 0 && <ul id="search-suggestions" role="listbox" aria-label="Recent search suggestions" className="absolute z-10 mt-1 w-full rounded-md border border-outline bg-surface p-1 shadow-xl">{suggestions.map((item, index) => <li id={`search-suggestion-${index}`} role="option" aria-selected={index === selectedSuggestion} key={item.id}><button type="button" className="w-full rounded px-3 py-2 text-left text-sm hover:bg-surface-elevated" onClick={() => { updateQuery(item.query); setSuggestionsOpen(false); }}>{item.query}</button></li>)}</ul>}</div>
      </header>
      ...
      <section className="mt-12" aria-live="polite">
        {!debouncedQuery ? (
          ...
          <>
            ...
            <AnimeGrid anime={results.anime} loading={(results.loading || pending) && results.anime.length === 0} emptyMessage={`No anime found for '${debouncedQuery}'. Try another title or spelling.`} />
