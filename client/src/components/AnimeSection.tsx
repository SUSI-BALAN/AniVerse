...
export function AnimeSection({ title, subtitle, anime, loading, viewAllLink, error, onRetry, reasons }: AnimeSectionProps) {
  ...
  return (
    <section aria-busy={loading} className="py-8" aria-labelledby={`section-${title.replaceAll(" ", "-").toLowerCase()}`}>
      <div className="mb-5 flex items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-1">
          ...
          {viewAllLink && <Link to={viewAllLink} className="ml-2 shrink-0 text-sm font-semibold text-accent-secondary hover:text-foreground">View all</Link>}
        </div>
      </div>
      {error ? <ErrorState message="This collection is unavailable right now." onRetry={onRetry} compact /> : !loading && anime.length === 0 ? <EmptyState icon={Clapperboard} title="No titles available" description="This collection does not have any anime to show right now." /> : (
        <div ref={rail} role="group" className="scrollbar-none -mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-3 sm:-mx-6 sm:gap-4 sm:px-6 lg:mx-0 lg:px-0" tabIndex={0} aria-label={`${title} anime`}>
          {loading
