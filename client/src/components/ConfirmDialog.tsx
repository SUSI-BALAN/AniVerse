...
export function ConfirmDialog({ title, description, cancelLabel = "Cancel", confirmLabel = "Confirm", danger = false, loading = false, requireText, onConfirm, onCancel }: ConfirmDialogProps) {
  ...
  const dialogRef = useRef<HTMLDivElement>(null); const previouslyFocused = useRef<HTMLElement | null>(null);
  const [typed, setTyped] = useState("");
  const enabled = !loading && (!requireText || typed === requireText);
  useEffect(() => {
    previouslyFocused.current = document.activeElement as HTMLElement | null;
    const dialog = dialogRef.current;
    dialog?.querySelector<HTMLElement>("input, button, [href], [tabindex]:not([tabindex='-1'])")?.focus();
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !loading) { event.preventDefault(); onCancel(); return; }
      if (event.key !== "Tab" || !dialog) return;
      const items = [...dialog.querySelectorAll<HTMLElement>("button:not(:disabled), input:not(:disabled), [href], [tabindex]:not([tabindex='-1'])")];
