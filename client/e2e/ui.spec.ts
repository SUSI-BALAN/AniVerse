...
async function mockMetadata(page: Page) {
  ...
  await page.route("**/api/recommendations**", (route) => route.fulfill({ json: { success: true, data: new URL(route.request().url()).pathname.endsWith("/home") ? { recommendations: [], because: { sourceAnime: { id: 1, title: "Frieren: Beyond Journey's End", coverImage: "https://images.test/cover-0.png" }, recommendations: anime.slice(2, 6).map(item => ({ anime: item, reason: "Because you watched Frieren" })) }, continuations: [], meta: { personalized: false, partial: false, failedSources: 0 } } : new URL(route.request().url()).pathname.endsWith("because-you-watched") ? { sourceAnime: { id: 1, title: "Frieren: Beyond Journey's End", coverImage: "https://images.test/cover-0.png" }, recommendations: anime.slice(2, 6).map((item) => ({ anime: item, reason: "Recommended for viewers of Frieren" })) } : [] } }));
  await page.route('**/api/home*', route => route.fulfill({ json: { success: true, data: { profile: { status: 'ready', data: { displayName: null } }, continueWatching: { status: 'ready', data: [] }, recentlyCompleted: { status: 'ready', data: [{ anilistId: 1, episodeNumber: 3, currentTime: 100, duration: 100, percentage: 100, completed: true, title: "Frieren: Beyond Journey's End", coverImage: 'https://images.test/cover-0.png', completedAt: '2026-01-01T00:00:00Z' }] }, personalized: { status: 'ready', data: { recommendations: [], continuations: [], because: { sourceAnime: { id: 1, title: "Frieren: Beyond Journey's End" }, recommendations: anime.slice(2,6).map(item => ({ anime: item, reason: 'Because you watched Frieren' })) }, favoriteGenres: null, audience: 'established', meta: { personalized: true, partial: false, failedSources: 0 } } } } } }));
