...
export function createUserRepositories(sqlite?: Database.Database): UserRepositories {
  if (env.DATABASE_MODE === "postgres") {
    ...
    return { queries: new PostgresLibraryQueryRepository(pool), library: new PostgresUserLibraryRepository(pool), playback: new PostgresUserPlaybackRepository(pool), profile: new PostgresUserProfileRepository(pool) };
  }
  const db = sqlite ?? getDatabase();
  return { queries: new SQLiteLibraryQueryRepository(db), library: new SQLiteUserLibraryRepository(db), playback: new SQLiteUserPlaybackRepository(db), profile: new SQLiteUserProfileRepository(db) };
}
